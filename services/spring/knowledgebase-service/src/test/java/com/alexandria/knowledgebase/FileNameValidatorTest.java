...
class FileNameValidatorTest {
    ...
    @ParameterizedTest
    @ValueSource(
            strings = {"report.pdf", "quarterly-report_v2.docx", "notes 2024.txt", "resume (final).pdf", "a.b.c.gz"
            })
    ...
    void unit_fnv_rejectsNull() {
        assertThatThrownBy(() -> FileNameValidator.validate(null)).isInstanceOf(IllegalArgumentException.class);
    }

    @ParameterizedTest
    @ValueSource(strings = {"", " ", "\t", "\n"})
    void unit_fnv_rejectsBlank(String name) {
        assertThatThrownBy(() -> FileNameValidator.validate(name)).isInstanceOf(IllegalArgumentException.class);
    }

    @ParameterizedTest
    @ValueSource(
            strings = {"../etc/passwd", "..", ".", "a/b.txt", "a\\b.txt", "evil\u0000.pdf", "bell\u0007.pdf", "del\u007F.pdf", "html<script>.txt", "pipe|rm.txt", "quote\".txt", "star*.txt", "colon:name.txt", "question?.txt"
            })
    ...
    void unit_fnv_rejectsOversizedNames() {
        ...
        assertThatThrownBy(() -> FileNameValidator.validate(longName)).isInstanceOf(IllegalArgumentException.class);
    }
