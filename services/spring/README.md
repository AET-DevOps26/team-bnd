...
- **Tags** (`POST /genai/tag`): content-based tags are autogenerated and persisted. The owner's existing tag labels are passed as `knownTags` so the model reuses them instead of producing near-duplicates.
...
### Generated GenAI client

The Gradle `openApiGenerate` task (OpenAPI Generator with the `restclient` library) produces the Request/Response models in `com.alexandria.genai.client.*` from the merged `api/openapi.yaml` during the build, and `compileJava` depends on that task,
so it is regenerated before any code that uses it compiles (see [`build.gradle`](./build.gradle)). The spec it generates from is itself kept in sync by the pre-commit hooks (`generate-genai-openapi.sh` and `merge-openapi.sh`), so an API change on the GenAI side flows through to this client without anyone editing it by hand.
