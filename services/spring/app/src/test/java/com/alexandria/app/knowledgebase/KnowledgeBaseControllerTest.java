...
class KnowledgeBaseControllerTest {
  ...
  void integration_controller_listDocumentsReturnsJson() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/knowledgebase/documents").with(jwt().jwt(j -> j.subject("sub123"))))
  ...
  void integration_controller_getDocumentReturnsJson() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/knowledgebase/documents/" + doc.getId())
  ...
  void integration_controller_getDocumentReturns404ForNonexistent() throws Exception {
    mockMvc
        .perform(
            get("/api/v1/knowledgebase/documents/" + java.util.UUID.randomUUID())
                .with(jwt().jwt(j -> j.subject("sub123"))))
  ...
  void integration_controller_downloadReturnsFileBytes() throws Exception {
    MockMultipartFile file =
        new MockMultipartFile("file", "download.txt", "text/plain", "hello world".getBytes());
    ...
    mockMvc
        .perform(
            get("/api/v1/knowledgebase/documents/" + doc.getId() + "/download")
                .with(jwt().jwt(j -> j.subject("sub123"))))
        .andExpect(status().isOk())
