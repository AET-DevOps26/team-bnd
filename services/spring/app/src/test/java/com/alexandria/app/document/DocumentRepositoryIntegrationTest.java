...
class DocumentRepositoryIntegrationTest {
    ...
    void integration_docRepo_findByOwnerIdWorks() {
        Document doc1 = new Document(testUser, "presentation.pptx", "/files/presentation.pptx", "application/vnd.openxmlformats-officedocument.presentationml.presentation", 4096L);
        ...
        assertThat(found).extracting(Document::getFileName).containsExactlyInAnyOrder("presentation.pptx", "data.csv");
    }
    ...
    void integration_docRepo_findByOwnerIdReturnsEmptyForDifferentUser() {
        ...
        Document document = new Document(testUser, "secret.txt", "/files/secret.txt", "text/plain", 256L);
    ...
    void integration_docRepo_findByOwnerIdAndFileNameContainingIgnoreCaseWorks() {
        Document doc1 = new Document(testUser, "report_2026.pdf", "/files/report.pdf", "application/pdf", 1024L);
        Document doc2 = new Document(testUser, "REPORT_summary.pdf", "/files/summary.pdf", "application/pdf", 2048L);
        ...
        List<Document> found = documentRepository.findByOwnerIdAndFileNameContainingIgnoreCase(
                testUser.getId(), "report");

        assertThat(found).hasSize(2);
        assertThat(found).extracting(Document::getFileName)
                .containsExactlyInAnyOrder("report_2026.pdf", "REPORT_summary.pdf");
    }

    @Test
    void integration_docRepo_existsByIdAndOwnerIdReturnsTrue() {
        Document document = new Document(testUser, "image.jpg", "/files/image.jpg", "image/jpeg", 8192L);
        Document saved = documentRepository.save(document);
    ...
    void integration_docRepo_existsByIdAndOwnerIdReturnsFalseForWrongOwner() {
        ...
        Document document = new Document(testUser, "diagram.png", "/files/diagram.png", "image/png", 4096L);
        Document saved = documentRepository.save(document);
    ...
    void integration_docRepo_existsByIdAndOwnerIdReturnsFalseForNonexistentDocument() {
        boolean exists = documentRepository.existsByIdAndOwnerId(
                java.util.UUID.randomUUID(), testUser.getId());
    ...
    void integration_docRepo_deleteRemovesDocument() {
        Document document = new Document(testUser, "temp.json", "/files/temp.json", "application/json", 128L);
        Document saved = documentRepository.save(document);
    ...
    void integration_docRepo_rawTextContentCanBeStored() {
        Document document = new Document(testUser, "article.html", "/files/article.html", "text/html", 3072L);
        ...
        assertThat(found).isPresent();
        assertThat(found.get().getRawTextContent()).isEqualTo("This is the extracted text content from the HTML document.");
    }

    @Test
    void integration_docRepo_multipleDocumentsForSameUserWorks() {
        String[] extensions = {"pdf", "docx", "txt", "xlsx", "md"};
        String[] mimeTypes = {"application/pdf", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "text/plain", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "text/markdown"};
        for (int i = 0; i < 5; i++) {
            Document doc = new Document(testUser, "doc" + i + "." + extensions[i], "/files/doc" + i + "." + extensions[i], mimeTypes[i], 1024L * (i + 1));
