...
Keycloak is the OIDC provider. The client redirects users to it for login/registration, and every Spring service validates the resulting JWT (via `OidcUserFilter` on user-service, which also creates the user record on first authenticated request). The realm is defined in `infra/oidc/realm.json`. Tokens are passed as Bearer headers on API calls.

Team member responsible for this subsystem: Niklas Ladurner

### Infrastructure, Deployment and CI/CD

The system is containerised and runs end-to-end via `docker-compose.yml` locally. The client and GenAI service each have their own Dockerfile. The three Spring services share a single parameterised one (`services/spring/Dockerfile`, selected per service via `--build-arg SERVICE=<name>`). The database and other infrastructure (Keycloak, Weaviate, object storage, Traefik, Prometheus, Grafana) run from stock upstream images. The system deploys to Kubernetes through a Helm chart (`infra/k8s/`) and to an Azure VM through Terraform + Ansible (`infra/azure/`). GitHub Actions runs CI on every PR (build, test, lint, OpenAPI checks) and deploys on merge to main.
