...
💡 **Tip**: Add a Git hook (`post-checkout`, `post-merge`) to run `./api/scripts/gen-all.sh` automatically.
This ensures generated clients are always up-to-date after switching branches or pulling changes.
...
- Pass JWTs through HTTP headers or cookies. Which option you choose has different advantages and disadvantages which need to be considered when making the decision.
...
| Observability       | Spring Boot Actuator + Prometheus; Include LangChain's tracing to OpenTelemetry; Scrape via Docker Compose       | Production-grade insights with minimal code.                              |
